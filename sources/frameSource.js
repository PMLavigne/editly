...
async function createFrameSource({ clip, clipIndex, width, height, channels, verbose, logTimes, ffmpegPath, ffprobePath, enableFfmpegLog, framerateStr }) {
  ...
  async function readNextFrame({ time, totalElapsedTime, clipFrame, globalFrame }) {
    ...
    for (const { frameSource, layer } of layerFrameSources) {
      ...
      if (shouldDrawLayer) {
        ...
        const rgba = await frameSource.readNextFrame(offsetProgress, canvas, totalElapsedTime, clipFrame, globalFrame);
